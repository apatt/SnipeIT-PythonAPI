...
class Assets(object):
    ...
    def delete(self, server, token, DeviceID):
        ...
        """
        self.uri = '/api/v1/hardware/'
        self.server = '%s%s%s' % (server, self.uri, DeviceID)
        headers = {'Content-Type': 'application/json','Authorization': 'Bearer ' + token}
    ...
    def updateDevice(self, server, token, DeviceID, payload):
        ...
        self.server = '%s%s%s' % (server, self.uri, DeviceID)
        headers = {'Content-Type': 'application/json','Authorization': 'Bearer ' + token}
        results = requests.patch(self.server, headers=headers, data=json.dumps(payload))
        return results.content
    ...
    def checkout(self, server, token, DeviceID, payload):
        ...
        Returns:
            [string] -- Server response in JSON formatted
        """
        self.uri = '/api/v1/hardware/%s/checkout' % DeviceID
        self.server = '%s%s' % (server, self.uri)
        headers = {'Content-Type': 'application/json','Authorization': 'Bearer ' + token}
        results = requests.post(self.server, headers=headers, data=json.dumps(payload))
        return results.content
